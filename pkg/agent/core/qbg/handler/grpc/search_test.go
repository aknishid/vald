...
func Test_server_Search(t *testing.T) {
	...
	defaultBeforeFunc := func(ctx context.Context, f fields, a args) (Server, error) {
		return buildIndex(ctx, f.objectType, f.distribution, a.insertNum, defaultInsertConfig, f.qbgCfg, f.qbgOpts, nil, f.overwriteVec)
	}
	...
	qbgConfig := func(dim int, objectType string) *config.NGT {
		return &config.NGT{
			Dimension:        dim,

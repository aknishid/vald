...
func Test_server_Insert_Update_parallel(t *testing.T) {
	...
	start := make(chan struct{}) // control the execution of goroutines
	wg := &sync.WaitGroup{}
...
func Test_server_Insert_Upsert_parallel(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	var successUpsertCnt int32 = 0
	for i := 0; i < upsertCnt; i++ {
		go func() {
			<-start // wait for start channel is closed to start this goroutine at the same time (as close as it can)
			defer wg.Done()

			_, err := s.Upsert(ctx, upsertReq) // upsert the request
			if err == nil {
				atomic.AddInt32(&successUpsertCnt, 1) // add successs count 1 if no error is returned
			}
		}()
	}
...
func Test_server_Update_Upsert_parallel(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	for i := 0; i < updateCnt; i++ {
		go func() {
			...
			_, err := s.Update(ctx, updateReq) // update the same update request
			if err == nil {
				atomic.AddInt32(&successUpdateCnt, 1) // add successs count 1 if no error is returned
			}
		}()
...
func Test_server_Insert_Update_Upsert_parallel(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
